...
def quant_int8_infer_impl(
    ...
    batch, seqlen, dims = input_tensor.shape
    ...
    try:
        ...
        num_programs = driver.active.utils.get_device_properties(
...
def scale_dynamic_quant_kernel_3d(
    ...
    Args:
        input: Pointer to input tensor with shape [B, S, D]
    ...
    for task_id in range(pid, num_tasks, grid_size):
        ...
        for col_block_offset in range(0, align_dims, BLOCK_SIZE_N):
            ...
            quant_vals_int8 = tl.cast(
                quant_vals, dtype=tl.int8, overflow_mode="saturate"
            )
